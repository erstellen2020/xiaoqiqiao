...
class App:
    ...
    def __init__(self):
        ...
        """
        # 定义好驱动
        self.driver = webdriver.Chrome(executable_path="./chromedriver")  # MAC系统 Chrome Webdriver
        # self.driver = webdriver.Chrome(executable_path='chromedriver.exe') # Windows系统 Chrome Webdriver
        # 标题层级，一个H标题为一层
        self.title_level = 0
        ...
        f = open("UserPass.txt")  # 以`用户名\n密码\n`的形式读取文件
        self.user_key = f.readline()[:-1]  # 去除换行
        # 用户名
        self.user_name = f.readline()[:-1]  # 去除换行
        # 用户密码
        ...
        try:
            for step in step_all:
                ...
                self.driver.find_element_by_xpath(
                    "/html/body/div[3]/div/div[2]/div/div[2]/div/div[2]/button[1]").click()
    ...
    def compress_image(self, infile, limit_size=500) -> str:
        ...
        if o_size <= limit_size:
            return infile
        outfile = os.path.join(os.getcwd(), 'new_' + str(infile.split("\\" and "/")[-1]))

        # 此处只压缩一次，基本满足要求
        if o_size >= limit_size:
            im = Image.open(infile)
            x, y = im.size
            im = im.resize((x // 2, y // 2), Image.ANTIALIAS)  # 缩小图片为1/4
            im.save(outfile, quality=85, subsampling=0)  # 设置质量为85%
        return outfile
    ...
    def chick_send(self):
        ...
        self.driver.find_element_by_xpath(self.xpath).send_keys(pyperclip.paste())
        # sleep(1)
    ...
    def md_analysis(self, expression):
        ...
        # 得到图片
            ...
            if not re.match("http", expression):
                # 本地图片直接提取路径
                print("NON_HTTP", expression)
                expression = os.path.abspath(expression)
            else:
                # 网络图片链接会下载到本地并拷贝路径
                print("HTTP:", expression)
                out_path = os.path.join(os.getcwd(), str(expression.split("\\" and "/")[-1]))
                Image.open(io.BytesIO(urlopen(expression).read())).save(out_path)
                expression = out_path
